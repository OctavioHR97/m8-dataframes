...
name.pop2013 <- names.2013[prop.2013 == max(prop.2013)]
...
MostPopular <- function(my.year){
  name.input <- name[year == my.year]
  prop.input <- prop[year == my.year]
  popular.name <- name.input[prop.input == max(prop.input)]
  return(popular.name)
}

# What was the most popular female name in 1994?
pop.name.2014 <- MostPopular(1994)
...
HowPopular <- function(my.name, my.year){
  names.input <- name[year == my.year]
  prop.input <- prop[year == my.year]
  probability <- prop.input[names.input == my.name]*100
  return(paste("The percentage of people named", my.name, "in the year", my.year, "was", probability))
}

# How popular was the name 'Laura' in 1995
pop.Laura <- HowPopular('Laura',1995)
